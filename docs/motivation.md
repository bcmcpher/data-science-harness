...
> has been measured.** Twenty-two plugins and 77 skills are on disk, structurally checked, and
...
credit as schema-validated state — with activity recorded once, as `DSH-*` lines in the commit that
did the work, rather than in a second log that could disagree with it — and a **living compendium** as the default export — a provenanced
...
| Capability plane | **Built, unexercised.** `datalad` has a toolbox of one skill with 19 verb references, used natively by the main thread; `nipoppy` four, one per command class; `annotate` one per backend (Neurobagel, NIDM, ReproSchema, SNOMED), most of which validate terms rather than find them; `archive` one per backend (OSF, Zenodo, DataCite); `compendium` four — MyST, Jupyter Book, repo2data and MCP bundle scaffolding; `bids` one, wrapping whichever validator distribution is installed; `liab` two — pyinfra, whose plan path the test exercises when pyinfra is installed and whose apply path is exercised only by hand because testing it needs a disposable host, and forgejo, exercised only as far as its credential gate. `containers` three, split by job — authoring a Dockerfile from a pinned manifest, building it with Podman or Docker, converting it to the `.sif` a cluster runs. The larger caveat is not shape but execution: no deposit, no repo2data fetch and no bundle emission has been run live here, and no document has been built *through the compendium doer*, because those tools are not installed. (mystmd itself is a locked devDependency and `tests/check-paper.sh` runs a real `myst build --strict` over `paper/` in CI — that establishes the tool works, not that the plugin's path through it does.) What the tests establish for those is that each gate answers correctly and refuses correctly, not that the tool behind it works. **`containers` is the exception**: Docker, Podman and Apptainer are all present on the development machine, and the full path — build, the `docker-daemon://` failure, the `docker-archive://` workaround, `inspect`, `exec`, and tag-to-digest resolution — has been run end to end. The comparison worth drawing is with `datalad`, which is a hard precondition of the e2e suite rather than a gated branch of it and is exercised against the real tool on every run — so `containers` is the exception among the *gated* capabilities, not the only real-tool path in the repository. Note also that the containers path is the one CI cannot run: apptainer is deliberately not installed there. |
| Ledger | **Built.** Schema-validated state, with a worked example. Activity is read from commit `DSH-*` lines by `dsh-log`. |
