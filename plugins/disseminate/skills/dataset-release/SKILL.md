...
citable record (Metadata). You own the release judgment; you tag and save it yourself, and delegate
DOI minting to the **archive doer**.
...
2. **Ensure a clean tree** — a release must tag committed state: check the dataset status (already
   in context, or run `datalad status`). If dirty, route to `analyze/checkpoint` first.
...
4. **Tag the release** — save with a version tag:
   ```bash
   datalad save -m "$(printf 'release <id> v<version>\n\nDSH-Op: dataset-release\nDSH-Stage: disseminate\nDSH-Product: <id>')" \
...
5. **Mint a DOI (archive doer, gated)** — delegate:
   ...
   - `result: ok` → capture the `doi` and the doer's `binding`; set it in `dataset_description.json`
     and (step 6) the ledger.
   - `result: unminted` → proceed without a DOI; report how to enable minting. Never invent one.
6. **Update the ledger and save** — set the product's `status: released`; append any minted DOI to
   the product's `dois[]` (per `docs/project-ledger.md`); save:
   ```bash
   datalad save -m "$(printf 'dataset-release: record DOI for <id> v<version>\n\nDSH-Op: dataset-release\nDSH-Stage: disseminate\nDSH-Product: <id>\nDSH-Binding: <backend>@<version>')"
   ```
   Copy `DSH-Binding` from the archive doer's report (e.g. `archive/zenodo@<version>`); omit the
   line when no DOI was minted.
...
- Run DataLad yourself (tag, save); delegate every deposit/mint to the archive doer — never call an
