...
anyone with access can clone it and retrieve the exact annexed outputs. Every sibling/push/verify
operation runs directly, in the main thread.
...
## Steps
1. **Ensure a clean, committed tree** — check the dataset status (already in context, or run
   `datalad status`). Push transfers only committed state. If the tree is dirty, tell the user and
   route to `analyze/checkpoint` first (or have them confirm publishing the current committed
   state).
2. **Choose the target sibling** — run `datalad siblings` to list configured siblings.
   - **A suitable sibling already exists** → use it.
   - **None exists** → ask the user where to publish, then create it yourself. Keep it to the
     lightest option that fits (baseline D needs only one of these):
     - local path / SSH host — `datalad create-sibling` (simplest, satisfies D)
     ...
     - OSF (`datalad create-sibling-osf`, needs the `datalad-osf` extension) or a RIA store —
       richer archival add-ons.
   ...
   consumers cannot retrieve outputs. Confirm with the user before creating a sibling.
3. **Guard against publishing secrets** — before pushing, confirm no credentials, tokens, or
   ...
   doubt, ask the user.
4. **Push** — confirm with the user, then:
   ```bash
   datalad push --to <sibling> --data anything
   ```
   (or `--data auto-if-wanted` if the sibling has `annex-wanted` configured); push all comparison
   branches, not just the current one.
5. **Verify Distributability** — the same proof the e2e smoke test asserts (see
   `tests/e2e-smoke.sh`, "distributability"): clone `<sibling>` to a scratch dir and `datalad get`
   one published output; confirm the content is retrievable, then discard the clone. If retrieval
   fails, the annexed content did not reach storage — most often a missing `--publish-depends`;
   report the error and stop.
6. **Report** — the sibling, what was transferred (git + annex), that an independent clone can
   retrieve the outputs, and any richer target the user might add next (OSF/Zenodo DOI, RIA).

## Constraints
- Run every sibling/push/clone/get operation yourself — never `git push`; it bypasses annex
  content and publish-depends ordering.
...
- Confirm with the user before pushing or creating a sibling.
