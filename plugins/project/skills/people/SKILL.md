...
ledger `contributors[]`; you save it yourself with `datalad save`.
...
4. **Save and record in one step**:
   ```bash
   datalad save -m "$(printf 'people: credit <name> (<roles>)\n\nDSH-Op: people\nDSH-Stage: manage')" project.yaml dataset_description.json
   ```
5. **Report** — the updated contributor list with roles, and any missing ORCIDs/affiliations to fill.
...
- Upsert per the ledger conventions and keep the ledger schema-valid. Record activity in the
  commit's `DSH-*` lines, never in `project.yaml` `log`; save with `datalad save` yourself.
